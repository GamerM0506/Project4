...
class GroupJoinRequestsTab extends StatelessWidget {
  ...
  const GroupJoinRequestsTab({Key? key, required this.groupId}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (_) => sl<GroupJoinRequestsCubit>()..fetchRequests(groupId, status: 'pending'),
            ...
            return ListView.separated(
              ...
              itemBuilder: (context, index) {
                ...
                return ListTile(
                  ...
                  title: Text(request.userId), // Should show User Name (Backend needs to join it or we fetch it)
                  subtitle: Text(
                  ...
                  trailing: Row(
                    ...
                    children: [
                      IconButton(
                        ...
                        onPressed: () {
                          context.read<GroupJoinRequestsCubit>().approveRequest(groupId, request.id);
                        },
                      ),
                      IconButton(
                        icon: const Icon(Icons.close, color: Colors.red),
                        onPressed: () {
                          context.read<GroupJoinRequestsCubit>().rejectRequest(groupId, request.id);
                        },
                      ),
                    ],
                  ),
