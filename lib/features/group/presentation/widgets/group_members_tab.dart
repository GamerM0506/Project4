...
class GroupMembersTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (_) => sl<GroupMembersCubit>()..fetchMembers(groupId, status: 'approved'),
              ...
              itemBuilder: (context, index) {
                ...
                return ListTile(
                  ...
                  trailing: member.role != 'owner' ? PopupMenuButton<String>(
                    ...
                    onSelected: (value) {
                      if (value == 'promote') {
                        context.read<GroupMembersCubit>().updateRole(groupId, member.userId, 'moderator');
                      } else if (value == 'demote') {
                        context.read<GroupMembersCubit>().updateRole(groupId, member.userId, 'member');
                      } else if (value == 'kick') {
                        showDialog(
                          context: context,
                          builder: (ctx) => AlertDialog(
                            title: const Text('Kích thành viên'),
                            content: const Text('Bạn có chắc muốn kích thành viên này khỏi nhóm?'),
                            actions: [
                              TextButton(onPressed: () => Navigator.pop(ctx), child: const Text('Hủy')),
                              TextButton(
                                onPressed: () {
                                  Navigator.pop(ctx);
                                  context.read<GroupMembersCubit>().kickMember(groupId, member.userId);
                                },
                    ...
                    itemBuilder: (context) => [
                      if (member.role != 'moderator')
                        const PopupMenuItem(
                          value: 'promote',
                          child: Text('Cấp quyền Kiểm duyệt (Moderator)'),
                        ),
                      if (member.role == 'moderator')
                        const PopupMenuItem(
                          value: 'demote',
                      ...
                      const PopupMenuItem(
                        value: 'kick',
                        child: Text('Kích khỏi nhóm', style: TextStyle(color: Colors.red)),
                      ),
                    ],
                  ) : const Text('Chủ nhóm', style: TextStyle(color: Colors.grey, fontWeight: FontWeight.bold)),
                );
              },
            );
          }
